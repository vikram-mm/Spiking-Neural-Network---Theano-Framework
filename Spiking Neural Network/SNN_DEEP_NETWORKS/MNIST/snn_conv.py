...
class snn():
    def __init__(self, input_shape, input = None,
        ...
        self.layers =[
                    ...
                    ('snn_conv1',snn_convLayer,[False,15,30,5,1]), #stdp_enabled,threshold,num_filter,filter_size,output_flag
	...
	f = open(os.path.join(path, 'snn_autonet_trained' + '.save'), 'rb')
	snn_loaded_object=cPickle.load(f)
	f.close()
	print('Done')
	
	self.copy_nets(snn_loaded_object.full_net_layers[-2],
                           full_net_layers[-2])
